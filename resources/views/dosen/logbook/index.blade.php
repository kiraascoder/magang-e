...
@section('page_desc', 'Review logbook mahasiswa bimbingan.')
...
@section('content')
    <div class="space-y-4">
        ...
        <form method="GET" action="{{ route('dosen.logbook') }}" class="flex flex-col sm:flex-row gap-2">
        ...
        <div class="bg-white border border-gray-200 rounded-2xl overflow-hidden">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse($items as $x)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-4 py-3">
                                    <a href="{{ route('dosen.logbook.show', $x->id_logbook) }}"
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-4 py-10 text-center text-gray-500">Belum ada logbook.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
            ...
            <div
                class="px-4 py-3 bord@extends('layouts.app')
...
<div class="space-y-4">
    ...
    <div class="bg-white border border-gray-200 rounded-2xl overflow-hidden">
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($items as $x)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3 text-gray-700">{{ $x->penempatan?->instansi?->nama_instansi ?? '-' }}</td>
                            <td class="px-4 py-3 text-gray-700">{{ $x->penempatan?->periode?->nama_periode ?? '-' }}</td>
                            <td class="px-4 py-3">
        ...
        <div class="px-4 py-3 border-t border-gray-200 flex items-center justify-between text-sm text-gray-600">
            <span>Menampilkan {{ $items->firstItem() ?? 0 }} - {{ $items->lastItem() ?? 0 }} dari {{ $items->total() }}</span>
            <div>{{ $items->links() }}</div>
        </div>
    </div>
</div>
@endsection
er-t border-gray-200 flex items-center justify-between text-sm text-gray-600">
