...
@section('page_title', 'Laporan Akhir Mahasiswa')
@section('page_desc', 'Review dan verifikasi laporan akhir mahasiswa bimbingan.')
...
@section('content')
    <div class="space-y-4">
        ...
        <form method="GET" action="{{ route('dosen.laporan_akhir') }}" class="flex flex-col sm:flex-row gap-2">
            <input type="text" name="q" value="{{ $q ?? '' }}" placeholder="Cari nama / NIM..."
                class="w-full sm:w-80 px-3 py-2 rounded-lg border border-gray-200">
            <select name="status" class="w-full sm:w-48 px-3 py-2 rounded-lg border border-gray-200">
        ...
        <div class="bg-white border border-gray-200 rounded-2xl overflow-hidden">
            <div class="px-4 py-3 border-b border-gray-200 flex items-center justify-between">
                <div class="font-semibold text-gray-900">Daftar Laporan Akhir</div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse($items as $x)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-4 py-3 text-gray-700">
                                    <a class="underline" target="_blank" href="{{ asset('storage/' . $x->file_path) }}">
                                        {{ $x->original_name ?? 'Lihat' }}
                                    </a>
                                    <div class="text-xs text-gray-500">
                                        {{ $x->file_size ? number_format($x->file_size / 1024, 0) : 0 }} KB
                                    </div>
                                </td>
                                ...
                                <td class="px-4 py-3">
                                    <a href="{{ route('dosen.laporan_akhir.show', $x->id_laporan_akhir) }}"
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-4 py-10 text-center text-gray-500">Belum ada laporan akhir.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
